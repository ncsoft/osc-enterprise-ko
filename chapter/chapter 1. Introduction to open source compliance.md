...
> 이전에는 기업 간 계약을 통해 라이선스 협상을 했으나, <br> 이제는 **컴플라이언스 정책과 개발 방식**으로 라이선스를 관리해야 함
<br>
...
| 문제 유형 | 확인 방법 | 예방법 |
|------|------|------|
| <img src="/image/chapter1-opensource-1.png" width="150"> <br> 오픈소스를 독점코드에 포함시키는 경우, <br> 개발자가 오픈소스 코드 스니펫만 복사하여 붙여넣기 했을 때 | 소스코드 스캔 | - 오픈소스 컴플라이언스에 대한 심각성 알리기 <br> - 정기적인 소스코드 스캔 <br> - 오픈소스 사전 승인 의무화 |
| <img src="/image/chapter1-opensource-2.png" width="150"> <br> 오픈소스를 독점 코드와 결합하는 경우, <br> 양립할 수 없는 라이선스를 결합시킬 때 | 디펜던시 추적 툴로 확인 | - 기업의 컴플라이언스 정책에 결합 방식을 가이드해야 함 <br> - 주기적으로 디펜던시 추적 툴로 결합방식 검증 |
| <img src="/image/chapter1-opensource-3.png" width="150"> <br> 독점 코드를 오픈소스 코드에 포함시키는 경우, 개발자가 독점 코드를 복사해서 붙여넣기 했을 때 | 소스코드 스캔 | - 사내 교육 <br> - 정기적인 소스코드 검사 <br> - 독점 코드를 포함시킬 때 사전 승인 |
...
<table>
  ...
  <tr>
    <td><br>소스코드를 배포하지 않음<br></td>
    <td><br>- 정식 배포되기 전에 체크리스트를 거쳐야 함<br></td>
  </tr>
  <tr>
    <td><br>배포된 버전과 일치하지 않은 소스코드를 배포했을 때<br></td>
    <td><br>- 소스코드를 배포할 때 정확히 일치한 버전인지 검증 절차를 거쳐야 함<br></td>
  </tr>
  <tr>
    <td><br>수정된 버전에 대한 오픈소스 라이선스를 공지하지 않았을 때<br></td>
    <td><br> - BOM관리 툴로 업데이트 버전과 기존 버전과의 차이점을 추적 및 관리해야 함<br>- 업데이트 버전에 대해 재 검증<br>- 체크리스트에 소스코드의 diff를 검증하는 단계를 추가해야 함<br></td>
  </tr>
  <tr>
    <td><br>수정된 소스코드를 표시하지 않거나 수정 고지를 하지 않았을 때<br></td>
    <td><br>- 소스코드 배포 전에 체크리스트로 확인 및 소스코드 검사<br>- 컴플라이언스 준수 절차에 수정된 소스코드를 확인하는 단계 넣기<br>- 개발자들에게 소스코드의 change log를 기록하도록 교육<br></td>
  </tr>
</table>
